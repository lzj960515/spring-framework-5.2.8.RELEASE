...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		//传入的是DefaultListableBeanFactory，true
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			//beanFactoryPostProcessors是空的，直接跳过
			...
			//下面的逻辑和上面的一模一样，while循环处理所有剩下的后置处理器
		...
		//上面处理完了所有实现BeanDefinitionRegistryPostProcessor接口的后置处理器
		//下面处理所有只实现了BeanFactoryPostProcessor接口的后置处理器
		//因为虽然BeanDefinitionRegistryPostProcessor继承了BeanFactoryPostProcessor，但是实现该接口的已经被处理完了
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		String[] postProcessorNames =
		...
		// 以下步骤存在疑问，为什么这里没有while循环处理？invokeBeanFactoryPostProcessors也是可以产生新的后置处理器的呀
	...
	private static final class BeanPostProcessorChecker implements BeanPostProcessor {
		...
		public Object postProcessAfterInitialization(Object bean, String beanName) {
			// 如果这个Bean不是个后置处理器，并且也不是个基础Bean, 而且后置处理器还没有被实例化完
			// 就打个日志记录下信息
